...
function createBodyParser(): Middleware {
...
async function parseBody(req: IncomingMessage, options: { limit: number }) {
  // defaults
...
function raw(stream: IncomingMessage, options: { limit: number; length: number }): Promise<string> {
...
function halt(stream: IncomingMessage) {
  stream.unpipe();
  stream.pause();
}

function readStream(stream: IncomingMessage, length: number, limit: number, callback: (err: any, val: string) => void) {
  let complete = false;
  let sync = true;
  ...
  function done(err: any, data?: string) {
    // mark complete
    complete = true;
